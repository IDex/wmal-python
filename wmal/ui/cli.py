...
class wmal_cmd(cmd.Cmd):
    ...
    def do_filter(self, arg):
        ...
        if arg:
            ...
            print("Available filters: %s" % ', '.join( v.lower().replace(' ', '') for v in list(self.engine.mediainfo['statuses_dict'].values()) ))
        ...
        if arg:
            try:
                ...
                if played_episode and playing_next:
                    do_update = input("Should I update %s to episode %d? [y/N] " % (show['title'], played_episode))
    ...
    def complete_filter(self, text, line, begidx, endidx):
        return (v.lower().replace(' ', '') for v in list(self.engine.mediainfo['statuses_dict'].values()))
    ...
    def _guess_status(self, string):
        for k, v in list(self.engine.mediainfo['statuses_dict'].items()):
...
class wmal_accounts(AccountManager):
    def select_account(self):
        while True:
            print('--- Accounts ---')
            self.list_accounts()
            key = input("Input account number ([a]dd, [c]ancel, [d]elete, [q]uit): ")
            ...
            if key.lower() == 'a':
                ...
                num = int(num)
                confirm = input("Are you sure you want to delete account %d (%s)? [y/N] " % (num, self.get_account(num)['username']))
                if confirm.lower() == 'y':
